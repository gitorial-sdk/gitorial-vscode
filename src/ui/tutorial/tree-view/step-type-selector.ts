...
.find(
				([_, info]) => `${info.icon} ${info.label}` === selected.label
			)?.[0] as StepType;
			...
			if (selectedType) {
				...
				vscode.window.showInformationMessage(
					`Step type changed to: ${STEP_TYPE_LABELS[selectedType].label}`
				);
			}
		}
	}
	 ...
	 */
	getFullCommitMessage(): string {
		return `${this.currentStepType}: ${this.currentStepMessage}`;
	}
	...
	static register(
		...
		const changeTypeDisposable = vscode.commands.registerCommand(
			'gitorial.changeStepType',
			() => selector.selectStepType()
		);

		const editMessageDisposable = vscode.commands.registerCommand(
			'gitorial.editStepMessage',
			() => selector.editStepMessage()
		);
